...
def direct_editor_video_data_url(video_path_text: str, modified_ns: int, frame_seconds: float, display_width: int) -> str:
    """Create a short muted clip for editing directly on moving uploaded video, not a still image."""
    del modified_ns
    ...
    handle.close()
    try:
        start = max(0.0, float(frame_seconds))
        width = max(180, min(590, int(display_width)))
        subprocess.run(
            [
                "ffmpeg", "-y", "-hide_banner", "-loglevel", "error", "-ss", f"{start:.2f}", "-i", str(source),
                "-t", "3.5", "-an", "-vf", f"fps=6,scale={width}:-2:flags=lanczos",
                "-c:v", "libx264", "-preset", "ultrafast", "-crf", "30", "-movflags", "+faststart", str(clip_path),
            ],
...
def render_paused_frame_editor(video_path: Path, duration_seconds: float) -> None:
    ...
    preview_width, preview_height = canvas_dimensions(Image.new("RGB", (output_width, output_height)), maximum_side=590)
    try:
    ...
    st.markdown('<div class="mk-editor-note">မူရင်း Video အတိုကလစ်ပေါ်မှာပါ။ <b>Blur Box နဲ့ Subtitle ကို လက်နဲ့ရွှေ့</b>ပြီး Box အစွန်းကိုဆွဲကာအရွယ်ပြောင်းပါ။ နေရာတည်ငြိမ်စေရန် ပြီးမှ <b>သိမ်းမယ်</b> ကိုနှိပ်ပါ။</div>', unsafe_allow_html=True)
