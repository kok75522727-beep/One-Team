...
def gemini_file_state_name(file_data) -> str:
    ...
    return str(state_name or "").strip().upper()
...
def generate_recap_script(video_path: Path, language: str, duration_seconds: int, tone: str, mode: str, progress_callback=None) -> str:
    ...
    try:
        if progress_callback:
            progress_callback(6, "Video file ကို AI ဆီပို့နေသည်")
        uploaded = call_gemini(lambda client: client.files.upload(file=str(media_path)))
        if progress_callback:
            progress_callback(10, "Video ကို AI စစ်ဆေးနေသည်")
...
def render_compact_voice_card_grid(
    ...
    """Render compact voice cards; one tap selects the card and plays its short sample."""
                ...
                if st.button(
                    ...
                    if access_error:
                        ...
                        try:
                            ...
                            st.session_state[f"{preview_state_key}_token"] = f"{model_name}:{time.time_ns()}"
                        except Exception as exc:
