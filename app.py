...
def validate_simple_free_api_key(api_key: str) -> tuple[bool, str]:
    ...
    try:
        ...
        result = client.interactions.create(model="gemini-2.5-flash", input="Reply only with READY")
        del result
...
def render_menu() -> None:
    with st.popover("⚙️ Setting", use_container_width=True):
        ...
        st.session_state.setdefault("gemini_text_model", "gemini-2.5-flash")
...
"""
    try:
        interaction = call_gemini(lambda client: client.interactions.create(
            model=st.session_state.get("gemini_text_model", "gemini-2.5-flash"),
...
FULL_VIDEO_SCRIPT_MODELS = ("gemini-2.5-flash",)
