...
def voice_provider_options(member: dict) -> list[str]:
    return ["Microsoft Premium", "Gemini"] if member_can_use_azure_voice(member) else ["Gemini"]


def selected_voice_provider(label: str) -> str:
    return "azure" if str(label) == "Microsoft Premium" else "gemini"
...
def voice_card_origin(model_name: str, provider: str) -> str:
    """Return a short, user-visible provider label without exposing any credential."""
    if provider == "azure":
        return AZURE_PREMIUM_VOICE_ORIGINS.get(model_name, "Microsoft Multilingual")
    return "Gemini အသံ"
...
def get_client() -> genai.Client:
    ...
    if not api_key:
        raise ValueError("Google AI Studio API Key မထည့်ရသေးပါ။ Simple Free Button ကိုနှိပ်ပြီးထည့်ပါ။")
...
def apply_compact_ui_theme() -> None:
    ...
    st.markdown(
        ...
        .voice-card-meta { min-height:1.65rem; padding:.18rem .2rem .32rem; color:#bfd0ec; text-align:center; font-size:.62rem; font-weight:700; line-height:1.15; }
        div[class*="st-key-voice_card_select_"] button { min-height:5.25rem; white-space:normal; line-height:1.25; border-radius:13px; border-color:#416794; background:linear-gradient(145deg,#192b49,#101d32); color:#f7fbff; font-size:.86rem; font-weight:900; box-shadow:inset 0 1px 0 rgba(255,255,255,.06); }
        div[class*="st-key-voice_card_select_"] button:hover { border-color:#74aaff; background:linear-gradient(145deg,#213a62,#142744); }
        div[class*="st-key-voice_card_preview_"] button { min-height:2rem; margin-top:-.1rem; border-radius:9px; border-color:#4e719e; color:#edf6ff; background:#13233b; font-size:.7rem; font-weight:900; }
        ...
        @media (max-width:720px) {
          ...
          div[class*="st-key-voice_card_select_"] button { min-height:4.85rem; padding:.16rem; font-size:.79rem; }
          .voice-card-meta { font-size:.58rem; }
        }
        </style>
        """,
        unsafe_allow_html=True,
...
def render_paused_frame_editor(video_path: Path, duration_seconds: float) -> None:
    ...
    editor_result = DIRECT_VIDEO_EDITOR(
        ...
        subtitle={"text": subtitle_text, "x": int(st.session_state.get("subtitle_x", SUBTITLE_DEFAULT_X)), "y": int(st.session_state.get("subtitle_y", SUBTITLE_DEFAULT_Y)), "color": st.session_state.get("subtitle_text_color", "#FFD166"), "size": int(st.session_state.get("subtitle_size", SUBTITLE_DEFAULT_SIZE))},
...
def voice_only_access_error(member: dict, provider: str = "gemini") -> str | None:
    """Use the correct protected owner-provider route for Voice Only output."""
    if provider == "azure":
        if not member_can_use_azure_voice(member):
            return "Microsoft Premium အသံက VIP / Owner အတွက်သာ ရပါတယ်။"
        return None if azure_speech_configured() else "Microsoft Azure Speech ကို Admin က Streamlit Secrets မှာ မပြင်ဆင်ရသေးပါ။"
    if bool(member.get("is_admin")):
...
def render_compact_voice_card_grid(
    ...
    """Render compact two-column selectable voice cards; preview never changes selection."""
    ...
    for start in range(0, len(cards), 2):
        ...
        for index, (column, (avatar, display_name, model_name)) in enumerate(zip(columns, cards[start:start + 2]), start=start):
            with column:
                ...
                if st.button("▶ နားထောင်", key=f"voice_card_preview_{scope}_{index}", use_container_width=True):
...
def render_voice_only_workspace(member: dict) -> None:
    ...
    provider_label = st.radio("အသံအမျိုးအစား", voice_provider_options(member), horizontal=True, key="voice_only_provider_label")
    voice_provider = selected_voice_provider(provider_label)
    cards = voice_cards_for_provider(voice_provider)
    if voice_provider == "azure":
        st.caption("Microsoft Premium · မြန်မာ Nilar / Thiha + Multilingual အသံ 10 မျိုး")
    ...
    if st.session_state.get("voice_only_preview_audio"):
        st.markdown('<div class="mk-result"><div class="mk-title">အစမ်းနားထောင်ရန် · 10–15 စက္ကန့်</div>', unsafe_allow_html=True)
        st.audio(pcm_to_wav(st.session_state.voice_only_preview_audio), format="audio/wav")
        st.markdown('</div>', unsafe_allow_html=True)
...
def main() -> None:
    ...
    with tab_voice:
        ...
        if auto_recap_voice_provider == "azure":
            st.caption("Microsoft Premium · မြန်မာ Nilar / Thiha + Multilingual အသံ 10 မျိုး")
        selected_name, selected_model = render_compact_voice_card_grid(
            "auto_recap",
            member,
