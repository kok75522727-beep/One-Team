...
try:
    ...
    # Override the old helper even when it exists: current Streamlit Cloud
    # changed its image helper signature while the canvas component did not.
...
def fabric_frame_background(frame: Image.Image, width: int, height: int) -> dict:
    """Embed the paused frame into Fabric JSON, avoiding Cloud media-manager background URLs."""
...
def render_paused_frame_editor(video_path: Path, duration_seconds: float) -> None:
    """Render direct movable Blur and Subtitle overlays in the original video-preview position."""
    ...
    try:
        ...
        frame = cached_editor_frame(str(video_path), modified_ns, frame_time).convert("RGBA")
    except Exception as exc:
        st.error(f"Frame မထုတ်နိုင်သေးပါ: {exc}")
        return
    source_width, source_height = frame.size
    canvas_width, canvas_height = canvas_dimensions(frame, maximum_side=520)
    ...
    if subtitle_enabled:
        ...
        canvas_font_size = max(15, min(38, round(subtitle_size * canvas_height / source_height)))
        ...
        canvas_objects.append({
            "type": "text", "text": subtitle_text, "left": canvas_width * subtitle_x / 100,
            "top": canvas_height * subtitle_y / 100, "originX": "center", "originY": "center",
    ...
    st.markdown('<div class="mk-editor-note">မူရင်း Video ရပ်ထားတဲ့ Frame ပေါ်မှာပါ။ <b>Blur Box နဲ့ Subtitle ကို လက်နဲ့ရွှေ့</b> ပြီး အစွန်းကနေအရွယ်ပြောင်းပါ။</div>', unsafe_allow_html=True)
    result = st_canvas(
        width=canvas_width, height=canvas_height, drawing_mode="transform",
        initial_drawing={"version": "4.4.0", "objects": canvas_objects, "backgroundImage": fabric_frame_background(frame, canvas_width, canvas_height)}, background_color="#080d19",
        stroke_color="#66d8ff", fill_color="rgba(79, 171, 255, 0.18)",
        update_streamlit=True, display_toolbar=False,
        key=f"original_video_preview_canvas_{video_path.stat().st_mtime_ns}_{round(frame_time, 1)}_{len(st.session_state.get('blur_masks') or [])}_{subtitle_enabled}",
    )
    objects = (getattr(result, "json_data", None) or {}).get("objects", [])
    if blur_enabled:
        updated_masks = source_boxes_from_canvas({"objects": objects}, canvas_width, canvas_height, source_width, source_height)
        if updated_masks:
            st.session_state.blur_masks = updated_masks
    if subtitle_enabled:
        subtitle_object = next((item for item in objects if item.get("type") == "text"), None)
        if subtitle_object:
            center_x, center_y = object_center(subtitle_object)
            st.session_state.subtitle_x = max(0, min(100, round(center_x * 100 / canvas_width)))
            st.session_state.subtitle_y = max(0, min(100, round(center_y * 100 / canvas_height)))
    if blur_enabled:
        ...
        if st.button("Export Video", type="primary", use_container_width=True, key="compact_export_video"):
            ...
            if access_error:
                ...
                try:
                    ...
                    with preview_placeholder.container():
                        render_paused_frame_editor(Path(video_path_text), duration)
                    st.error(f"Video export မအောင်မြင်ပါ: {api_error_message(exc)}")
