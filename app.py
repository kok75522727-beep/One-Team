...
st.markdown(
    ...
    .stTabs [data-baseweb="tab-list"] { gap:.35rem; background:#0a1326; padding:.4rem; margin-top:.55rem; border:1px solid #33598d; border-radius:12px; }
    .stTabs [data-baseweb="tab"] { flex:1 1 0; justify-content:center; height:2.3rem; min-width:0; padding:0 .45rem; border-radius:8px; background:#162846; color:#e6f2ff !important; font-size:.78rem; font-weight:800; white-space:nowrap; }
    ...
    [data-testid="stFileUploaderDropzone"] * { color:#e9f5ff !important; }
    div[data-testid="stFileUploader"] button { color:#0b1324 !important; background:#aee8ff !important; border-color:#d5f7ff !important; }
    div[data-testid="stSelectbox"] div[data-baseweb="select"] > div, div[data-testid="stTextInput"] div[data-baseweb="input"] > div, div[data-testid="stNumberInput"] div[data-baseweb="input"] > div { background:#102447 !important; border-color:#6c9ce0 !important; color:#f4f8ff !important; }
    div[data-testid="stSelectbox"] div[data-baseweb="select"] * , div[data-testid="stTextInput"] input, div[data-testid="stNumberInput"] input { color:#f4f8ff !important; }
    ...
    @media (max-width: 720px) {
      ...
      .stTabs [data-baseweb="tab"] { padding:0 .22rem; font-size:.68rem; }
    }
    </style>
    """,
...
with right:
    ...
    if subtitle_on:
        ...
        c1, c2 = st.columns(2)
        c1.color_picker("Subtitle Color", "#FFFFFF", key="subtitle_color")
        c2.selectbox("Outline", ["None", "Thin", "Medium", "Thick"], key="outline")
        c1, c2 = st.columns(2)
...
with tab_logo:
    ...
    with logo_position_col:
        st.selectbox("Image Position", ["Left", "Right"], key="image_logo_position")
    with text_logo_col:
        st.text_input("Text Logo", placeholder="Enter logo text", key="text_logo")
    with text_motion_col:
        st.selectbox("Text Motion", ["Left", "Right", "Scroll"], key="text_logo_motion")
    st.markdown('</div>', unsafe_allow_html=True)
