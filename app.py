...
def generate_simple_rest_voiceover(prompt: str, voice: str) -> bytes:
    ...
    if not encoded:
        raise RuntimeError("Gemini 3.1 Voice က audio data မပြန်ပေးပါ။")
    try:
        return base64.b64decode(encoded)
    except Exception as exc:
...
def generate_segmented_voiceover(script: str, voice: str, style: str, progress_callback=None, provider: str = "gemini") -> bytes:
    ...
    member = st.session_state.get("current_member") or {}
    # Gemini 3.1 TTS is preview and may take longer for a huge narration block.
    # Smaller Simple chunks give visible progress and keep each own-key request bounded.
    max_characters = 850 if provider == "gemini" and str(member.get("effective_plan", "")) == "simple" and not bool(member.get("is_admin")) else 2600
