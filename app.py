...
try:
    ...
    create_client = None
try:
    import streamlit_drawable_canvas as _drawable_canvas_module
    from streamlit_drawable_canvas import st_canvas
    # streamlit-drawable-canvas still imports Streamlit's pre-1.50 image helper.
    # Restore that narrow adapter so the canvas can keep its video frame in the
    # media manager on current Streamlit deployments.
    from streamlit.elements.lib.image_utils import image_to_url as _streamlit_image_to_url
    from streamlit.elements.lib.layout_utils import LayoutConfig as _StreamlitLayoutConfig
    if not hasattr(_drawable_canvas_module.st_image, "image_to_url"):
        def _canvas_image_to_url(image, width, clamp, channels, output_format, image_id):
            return _streamlit_image_to_url(
                image,
                layout_config=_StreamlitLayoutConfig(width=width),
                clamp=clamp,
                channels=channels,
                output_format=output_format,
                image_id=image_id,
            )
        _drawable_canvas_module.st_image.image_to_url = _canvas_image_to_url
except ImportError:  # Shows a clear deployment message if requirements are not yet refreshed.
    st_canvas = None
...
SIMPLE_VIP_PRICE_MMK = "10,000 MMK"
...
def create_pending_member(identity: dict) -> dict | None:
    ...
    if client is None:
        return None
    try:
        created = client.table("members").insert(identity).execute().data or []
        member = dict(created[0]) if created else find_member(identity["google_subject"])
        if member:
            record_member_audit(identity["google_subject"], "registered", identity["email"])
        return member
    except Exception:
        return None
...
def pending_simple_payment_request(google_subject: str) -> dict | None:
    client = get_supabase_client()
    if client is None:
        return None
    try:
        rows = client.table("payment_requests").select("*").eq("google_subject", google_subject).eq("plan", "simple").eq("status", "submitted").order("submitted_at", desc=True).limit(1).execute().data or []
        return dict(rows[0]) if rows else None
    except Exception:
        return None
...
def create_simple_payment_request(member: dict, payment_method: str, transaction_id: str, receipt=None) -> tuple[bool, str]:
    ...
    existing = pending_simple_payment_request(str(member.get("google_subject", "")))
    if existing:
        return False, "ငွေလွှဲစစ်ဆေးရန်တောင်းဆိုထားပြီးပါပြီ။ Admin အတည်ပြုမှုကိုစောင့်ပါ။"
    ...
    try:
        ...
        record_member_audit(member["google_subject"], "payment_submitted", str(member.get("email", "")), f"Simple VIP via {payment_method}")
        return True, "ငွေလွှဲအချက်အလက်ပို့ပြီးပါပြီ။ Admin စစ်ဆေးပြီး Simple VIP ဖွင့်ပေးပါမယ်။"
    except Exception:
...
def approve_simple_payment(request_id: int, payment: dict, days: int, actor: str, note: str = "") -> bool:
    if not approve_member(str(payment.get("google_subject", "")), "simple", days, actor, note):
        return False
    client = get_supabase_client()
    if client is None:
        return False
    try:
        ...
        record_member_audit(str(payment.get("google_subject", "")), "payment_approved", actor, f"Simple VIP payment #{request_id}")
        return True
    except Exception:
        return False


def render_simple_payment_request(member: dict) -> None:
    """Manual collection only: the owner reviews a submitted ID/receipt before enabling Simple VIP."""
    with st.expander("Simple VIP · 10,000 MMK", expanded=True):
        st.caption("Simple VIP အတွက် ကိုယ်ပိုင် Gemini Free API Key လိုပါတယ်။ ငွေလွှဲပြီးမှ Admin စစ်ဆေးဖွင့်ပေးပါမယ်။")
        for method, details in SIMPLE_PAYMENT_DESTINATIONS.items():
            st.markdown(f"**{method}** · `{details['phone']}` · {details['account_name']}")
        submitted = pending_simple_payment_request(str(member.get("google_subject", "")))
        if submitted:
            st.info(f"ငွေလွှဲတောင်းဆိုမှု ပို့ပြီးပါပြီ။ {submitted.get('payment_method', '')} · ID: {submitted.get('transaction_id', '')}")
            return
        with st.form("simple_payment_request"):
            method = st.radio("ငွေလွှဲမည့် App", list(SIMPLE_PAYMENT_DESTINATIONS), horizontal=True)
            transaction_id = st.text_input("Transaction ID", max_chars=100)
            receipt = st.file_uploader("Receipt ပုံ (မထည့်လည်းရ)", type=["jpg", "jpeg", "png", "webp"], key="simple_payment_receipt")
            submitted_form = st.form_submit_button("ငွေလွှဲအချက်အလက် ပို့မယ်", type="primary", use_container_width=True)
        if submitted_form:
            success, message = create_simple_payment_request(member, method, transaction_id, receipt)
            if success:
                st.success(message)
                st.rerun()
            st.error(message)
...
def pro_export_used_today(google_subject: str, export_day: str | None = None) -> bool:
    ...
    if client is None:
        return True
    try:
        rows = client.table("export_usage").select("id").eq("google_subject", google_subject).eq("plan", "pro").eq("outcome", "success").eq("export_day", export_day or myanmar_export_day()).limit(1).execute().data or []
        return bool(rows)
    except Exception:
        return True
...
def export_access_error(member: dict, duration_seconds: float) -> str | None:
    ...
    if plan == "simple":
        return None if st.session_state.get("google_ai_key") else "Simple VIP အတွက် Menu ထဲမှာ ကိုယ်ပိုင် Gemini Free API Key ကိုအရင်ထည့်ပါ။"
    if plan == "pro":
        if not owner_api_key():
            return "Pro VIP Owner API Key ကို Admin က Streamlit Secrets မှာ မထည့်ရသေးပါ။"
        if duration_seconds >= 240:
            return "Pro VIP က ၄ မိနစ်အောက် Video ၁ ပုဒ်သာ ထုတ်လို့ရပါတယ်။"
        if pro_export_used_today(str(member.get("google_subject", ""))):
            return "Pro VIP ရဲ့ ဒီနေ့ Video ၁ ပုဒ် ထုတ်ပြီးပါပြီ။ မြန်မာအချိန် မနက်ဖြန်မှ ပြန်ထုတ်လို့ရပါမယ်။"
        return None
...
def register_successful_pro_export(member: dict, duration_seconds: float) -> bool:
    if str(member.get("effective_plan")) != "pro":
        return True
    client = get_supabase_client()
    if client is None:
        return False
    try:
        client.table("export_usage").insert({
            "google_subject": member["google_subject"], "plan": "pro", "export_day": myanmar_export_day(),
            "source_duration_seconds": round(float(duration_seconds), 2), "outcome": "success",
        }).execute()
        return True
    except Exception:
        return False
...
def render_member_admin() -> None:
    ...
    with st.expander("Admin Members"):
        ...
        st.caption(f"Simple VIP Payment Request: {len(payment_requests)}")
        if payment_requests:
            labels = {
                f"{member_lookup.get(str(row.get('google_subject')), {}).get('email', 'Unknown')} · {row.get('payment_method')} · {row.get('transaction_id')}": row
            ...
            payment_days = st.number_input("Simple VIP Days", min_value=1, max_value=366, value=30, key="simple_payment_days")
            payment_note = st.text_input("Payment Admin Note", key="simple_payment_note")
            if st.button("ငွေလွှဲစစ်ပြီး Simple VIP ဖွင့်မယ်", type="primary", use_container_width=True, key="simple_payment_approve"):
                if approve_simple_payment(int(selected_payment["id"]), selected_payment, int(payment_days), str(member.get("email", "")), payment_note):
                    st.success("Simple VIP ဖွင့်ပြီးပါပြီ။")
                    st.rerun()
        ...
        if active_members:
            ...
            if st.button("Suspend Selected", use_container_width=True, key="member_suspend"):
                if suspend_member(labels[selected_label]["google_subject"], str(member.get("email"))):
                    ...
                    st.rerun()
        st.caption("Pro VIP: ၄ မိနစ်အောက် Video ၁ ပုဒ် / မြန်မာနေ့စဉ်။ Simple VIP: User Key သီးသန့်။")
...
def render_account_gate() -> dict:
    ...
    if identity is None:
        ...
        st.caption("Google Account နဲ့ဝင်ပြီး Account ဖွင့်ပါ။ Admin အတည်ပြုပြီးမှ Video Export သုံးလို့ရပါမယ်။")
    ...
    if member is None:
        member = create_pending_member(identity)
        if member:
            st.success("Account ဖွင့်ပြီးပါပြီ။ Admin အတည်ပြုမှုကို စောင့်ပါ။")
            st.rerun()
    ...
    if not member_can_enter_editor(member):
        if member.get("status") == "suspended":
            ...
            st.info("Account ဖွင့်ပြီးပါပြီ။ Admin က စစ်ဆေးပြီး VIP ဖွင့်ပေးရန် စောင့်ပါ။")
            render_simple_payment_request(member)
        else:
...
def run_one_click_youtube_export(video_path: Path, duration_seconds: int, tone: str, mode: str, voice: str, voice_style: str, audio_speed: float, blur_enabled: bool, blur_masks: list[dict], blur_strength: int, blur_background: str, blur_color: str, export_style: dict, logo_path: str | None, logo_position: str, logo_motion: str, moving_logo_text: str, text_position: str, effect_auto_zoom: bool, effect_color_filter: bool, effect_pitch_alter: bool, subtitle_enabled: bool = True, progress_callback=None, timing_basis: str = "Audio အချိန်", effect_background_blur: bool = False, effect_freeze_bypass: bool = False, cached_script: str | None = None, cached_audio: bytes | None = None, target_platform: str | None = None, quality_mode: str | None = None, original_audio_mode: str = "မူရင်းအသံအကုန်ဖျောက်", background_music_path: str | None = None, background_music_volume: int = 12) -> tuple[bytes, str, str, bytes]:
    ...
    if audio:
        ...
        audio = generate_voiceover(script, voice, voice_style)
...
def apply_compact_ui_theme() -> None:
    ...
    st.markdown(
        ...
        div.st-key-simple_vip button { min-height:3rem; border:1px solid #ccefff; background:linear-gradient(135deg,#86d8ff,#6688ff); color:#09162b !important; font-size:.82rem; font-weight:900; }
        div.st-key-pro_vip button { min-height:3rem; border:1px solid #ffe39a; background:linear-gradient(135deg,#ffe081,#f49b52); color:#231506 !important; font-size:.82rem; font-weight:900; }
        ...
        iframe[title="streamlit_drawable_canvas.streamlit_drawable_canvas"] { border:1px solid #507cc1; border-radius:12px; background:#080d19; }
        @media (max-width:720px) {
          .block-container { padding:.65rem .75rem 1.4rem; }
          .mk-brand h1 { font-size:1.1rem; }
          .mk-brand p { font-size:.67rem; }
          .stTabs [data-baseweb="tab"] { padding:0 .32rem; font-size:.73rem; }
        }
        </style>
        """,
        unsafe_allow_html=True,
...
def render_compact_menu() -> None:
    """Keep credentials and admin controls out of the editor workspace."""
    with st.popover("☰ Menu", use_container_width=True):
        st.markdown("### Settings")
        member = st.session_state.get("current_member") or {}
        plan = member.get("effective_plan", "none")
        st.caption(f"{member.get('display_name', '')} · {str(plan).title()} VIP")
        if plan == "simple":
            st.caption("Simple VIP အတွက် ကိုယ်ပိုင် Gemini Free API Key ထည့်ပါ။ GitHub ထဲမသိမ်းပါ။")
            key_value = st.text_input(
                "API Key",
                type="password",
                value=st.session_state.get("google_ai_key", ""),
                key="compact_api_key",
                placeholder="Key ထည့်ပါ",
            )
            if key_value.strip():
                st.session_state.google_ai_key = key_value.strip()
            if get_api_key():
                st.success("Key ready")
            if st.button("Session Key ဖျက်မယ်", use_container_width=True, key="compact_clear_api_key"):
                st.session_state.pop("google_ai_key", None)
                st.rerun()
        else:
            st.caption("Pro VIP — Owner API ဖြင့် တစ်နေ့ ၄ မိနစ်အောက် Video ၁ ပုဒ် ထုတ်နိုင်ပါတယ်။")
...
def render_paused_frame_editor(video_path: Path, duration_seconds: float, platform: str, quality_mode: str) -> None:
    """Render manual Blur and Subtitle placement against a selected video frame."""
    ...
    if st_canvas is None:
        st.error("Frame editor ကိုဖွင့်ရန် app ကို Reboot လုပ်ပါ။")
        return
    ...
    try:
        ...
        frame = cached_editor_frame(str(video_path), modified_ns, frame_time)
    except Exception as exc:
        st.error(f"Frame မထုတ်နိုင်သေးပါ: {exc}")
        return
    source_width, source_height = frame.size
    st.markdown('<div class="mk-editor-note">Video ကိုရပ်ထားတဲ့ Frame ပါ။ <b>Blur Box ကို လက်နဲ့ဆွဲ/ပြောင်းအရွယ်လုပ်</b> ပါ။ Subtitle ကိုလည်း Frame ပေါ်မှာဆွဲရွှေ့နိုင်ပါတယ်။</div>', unsafe_allow_html=True)

    if blur_enabled:
        st.markdown('<div class="mk-title">Blur — စာတန်းဟောင်းဖျောက်ရန်</div>', unsafe_allow_html=True)
        defaults = st.session_state.get("blur_masks")
        ...
        blur_controls, blur_canvas_column = st.columns([0.72, 1.28], gap="medium")
        with blur_controls:
            if st.button("+ Blur Box", use_container_width=True, disabled=len(st.session_state.blur_masks) >= 3, key="compact_add_blur_box"):
            ...
            if st.session_state.get("blur_background_style") == "Solid Box":
                ...
                st.session_state.setdefault("solid_box_color", "#16B8FF")
            st.slider("Strength", 2, 40, int(st.session_state.get("blur_strength", 18)), key="blur_strength")
            st.caption("Box ကိုထိပြီး ရွှေ့၊ အစွန်းလေးတွေကိုဆွဲပြီး အကျယ်/အမြင့်ပြောင်းပါ။")
        with blur_canvas_column:
            canvas_width, canvas_height = canvas_dimensions(frame)
            ...
            drawing = {
                ...
                "objects": [
                    {
                        "type": "rect", "left": mask["x"] * scale_x, "top": mask["y"] * scale_y,
                        "width": mask["width"] * scale_x, "height": mask["height"] * scale_y,
                        "fill": "rgba(79, 171, 255, 0.18)", "stroke": "#66d8ff", "strokeWidth": 3,
                        "transparentCorners": False, "cornerColor": "#ffd166", "cornerSize": 13,
                    }
                    for mask in st.session_state.blur_masks[:3]
                ],
            }
            result = st_canvas(
                background_image=frame, width=canvas_width, height=canvas_height, drawing_mode="transform",
                initial_drawing=drawing, stroke_color="#66d8ff", fill_color="rgba(79, 171, 255, 0.18)",
                update_streamlit=True, display_toolbar=True, key=f"blur_frame_canvas_{video_path.stat().st_mtime_ns}_{round(frame_time, 1)}_{len(st.session_state.blur_masks)}",
            )
            updated_masks = source_boxes_from_canvas(getattr(result, "json_data", None), canvas_width, canvas_height, source_width, source_height)
            if updated_masks:
                st.session_state.blur_masks = updated_masks

    if subtitle_enabled:
        st.markdown('<div class="mk-title">Subtitle — စာတန်းနေရာ</div>', unsafe_allow_html=True)
        output_width, output_height = render_dimensions(platform, quality_mode)
        subtitle_canvas_image = fit_frame_for_export_canvas(frame, output_width, output_height, bool(st.session_state.get("effect_background_blur", False)))
        subtitle_canvas_width, subtitle_canvas_height = subtitle_canvas_image.size
        subtitle_text = first_srt_caption(st.session_state.get("generated_srt", "")) or "စာတန်းထိုး နေရာ"
        subtitle_text = " ".join(subtitle_text.split())[:55]
        subtitle_size = int(st.session_state.get("subtitle_size", 38))
        canvas_font_size = max(15, min(38, round(subtitle_size * subtitle_canvas_height / output_height)))
        subtitle_x = max(0, min(100, int(st.session_state.get("subtitle_x", 50))))
        subtitle_y = max(0, min(100, int(st.session_state.get("subtitle_y", 78))))
        subtitle_drawing = {
            "version": "4.4.0",
            "objects": [{
                "type": "text", "text": subtitle_text, "left": subtitle_canvas_width * subtitle_x / 100,
                "top": subtitle_canvas_height * subtitle_y / 100, "originX": "center", "originY": "center",
                "fontSize": canvas_font_size, "fontFamily": "Noto Sans Myanmar, sans-serif",
                "fill": st.session_state.get("subtitle_text_color", "#FFD166"),
                "stroke": st.session_state.get("subtitle_outline_color", "#000000"), "strokeWidth": 2,
                "backgroundColor": st.session_state.get("subtitle_background_color", "#000000") if st.session_state.get("subtitle_background_mode") == "Solid background" else "",
                "lockScalingX": True, "lockScalingY": True, "transparentCorners": False, "cornerColor": "#ffd166", "cornerSize": 13,
            }],
        }
        subtitle_result = st_canvas(
            background_image=subtitle_canvas_image, width=subtitle_canvas_width, height=subtitle_canvas_height,
            drawing_mode="transform", initial_drawing=subtitle_drawing, stroke_color="#ffd166", fill_color="rgba(0,0,0,0)",
            update_streamlit=True, display_toolbar=True, key=f"subtitle_frame_canvas_{video_path.stat().st_mtime_ns}_{round(frame_time, 1)}_{subtitle_size}_{output_width}x{output_height}",
        )
        subtitle_objects = (getattr(subtitle_result, "json_data", None) or {}).get("objects", [])
        if subtitle_objects:
            center_x, center_y = object_center(subtitle_objects[0])
            st.session_state.subtitle_x = max(0, min(100, round(center_x * 100 / subtitle_canvas_width)))
            st.session_state.subtitle_y = max(0, min(100, round(center_y * 100 / subtitle_canvas_height)))
        st.caption(f"Final position: X {st.session_state.get('subtitle_x', 50)} · Y {st.session_state.get('subtitle_y', 78)}")
...
def main() -> None:
    ...
    plan_label = "Simple VIP" if member.get("effective_plan") == "simple" else "Pro VIP"
    menu_col, brand_col, simple_vip_col, pro_vip_col = st.columns([0.78, 2.7, 1.25, 1.05], gap="small")
    with menu_col:
        render_compact_menu()
    with brand_col:
        st.markdown(f'<div class="mk-brand"><div class="mk-brand-mark">▶</div><div><h1>One Team</h1><p>{html_lib.escape(plan_label)} · Movie Recap Studio</p></div></div>', unsafe_allow_html=True)
    with simple_vip_col:
        st.button("Simple VIP · 10K", key="simple_vip", use_container_width=True, disabled=member.get("effective_plan") == "simple")
    with pro_vip_col:
        st.button("Pro VIP · 25K", key="pro_vip", use_container_width=True, disabled=member.get("effective_plan") == "pro")
    ...
    with editor_right:
        ...
        st.toggle("Subtitle", value=bool(st.session_state.get("subtitle_enabled", True)), key="subtitle_enabled")
    ...
    if video_path_text:
        ...
        render_paused_frame_editor(Path(video_path_text), duration, platform, quality_mode)
        if st.session_state.get("subtitle_enabled", True):
            with st.expander("Subtitle Style", expanded=False):
                ...
                st.caption("Subtitle Position ကို အပေါ်က Frame ပေါ်မှာ လက်နဲ့ရွှေ့ပါ။")
        st.session_state.subtitle_export_style = {
        ...
        st.divider()
        if st.button("Export Video", type="primary", use_container_width=True, key="compact_export_video"):
            ...
            if access_error:
                ...
                mode = "Faithful full translation" if source_kind == "Movie Recap" else "Original recap"
                progress = st.progress(0, text="Video ပြင်နေသည်...")
                try:
                    def update_progress(percent: int, label: str) -> None:
                        progress.progress(min(100, max(0, percent)), text=label)
                    ...
                    if not register_successful_pro_export(member, duration):
                        st.session_state.output_video = None
                        st.session_state.audio = None
                        st.warning("Pro VIP ရဲ့ ဒီနေ့ Limit ပြည့်သွားပါပြီ။ Final Video ကို မသိမ်းပါ။")
                        st.stop()
                    register_generation()
                    progress.progress(100, text="Video ပြီးပါပြီ")
