...
SIMPLE_FREE_DAILY_EXPORT_LIMIT = 3
...
def apply_compact_ui_theme() -> None:
    ...
    st.markdown(
        ...
        .voice-card-face { width:58px; height:58px; margin:0 auto; border:1px solid #42679d; border-radius:50%; background:radial-gradient(circle at 50% 18%,#395983 0%,#192a44 52%,#0c1424 100%); display:grid; place-items:center; font-size:2.2rem; line-height:1; overflow:hidden; box-shadow:inset 0 1px 0 rgba(255,255,255,.12); }
        ...
        div[class*="st-key-voice_only_card_"] button { min-height:2.2rem; margin-top:.24rem; white-space:normal; line-height:1.1; border-radius:8px; border-color:#416794; background:#162743; color:#f1f7ff; font-size:.7rem; font-weight:900; }
        div[class*="st-key-voice_only_card_"] button:hover { border-color:#5b9bff; background:#172b4a; }
        @media (max-width:720px) {
          ...
          div[class*="st-key-voice_only_card_"] button { min-height:2.15rem; padding:.06rem; font-size:.62rem; }
        }
        </style>
        """,
        unsafe_allow_html=True,
...
def voice_only_access_error(member: dict) -> str | None:
    """Use the same protected account-key route as the existing Auto Recap editor."""
    if bool(member.get("is_admin")):
...
def build_voice_only_audio(text: str, model_voice: str, style_label: str, speed: float, pitch_label: str) -> tuple[bytes, str]:
    ...
    audio = generate_segmented_voiceover(text, model_voice, style)
...
def render_voice_only_workspace(member: dict) -> None:
    ...
    st.session_state.setdefault("voice_only_name", VOICE_CARDS[0][1])
    for start in range(0, len(VOICE_CARDS), 5):
        columns = st.columns(5, gap="small")
        for index, (column, (avatar, display_name, model_name)) in enumerate(zip(columns, VOICE_CARDS[start:start + 5]), start=start):
            with column:
                st.markdown(f'<div class="voice-card-face sprite pos-{index}">{avatar}</div>', unsafe_allow_html=True)
                selected_mark = " ✓" if st.session_state.get("voice_only_name") == display_name else ""
                if st.button(f"{display_name}{selected_mark}", key=f"voice_only_card_{display_name}", use_container_width=True):
                    st.session_state.voice_only_name = display_name
                    st.session_state.voice_only_model = model_name
    selected_voice = next((entry for entry in VOICE_CARDS if entry[1] == st.session_state.get("voice_only_name")), VOICE_CARDS[0])
    st.session_state.voice_only_model = selected_voice[2]
    ...
    with actions_a:
        if st.button("▶ အစမ်းနားထောင်မယ်", key="voice_only_preview", use_container_width=True):
            access_error = voice_only_access_error(member)
            if access_error:
                ...
                try:
                    with st.spinner("အသံအစမ်းထုတ်နေသည်…"):
                        audio, _ = build_voice_only_audio(preview_text, selected_voice[2], style_label, speed, pitch_label)
            ...
            if access_error:
                ...
                try:
                    with st.spinner("အသံနဲ့ SRT ပြင်နေသည်…"):
                        audio, srt_text = build_voice_only_audio(voice_text, selected_voice[2], style_label, speed, pitch_label)
...
def main() -> None:
    ...
    with telegram_left:
        st.link_button("✈ One Team Channel", TELEGRAM_CHANNEL_URL, use_container_width=True)
    with telegram_right:
        st.link_button("✈ One Team Group", TELEGRAM_GROUP_URL, use_container_width=True)
    ...
    if video_path_text:
        ...
        if st.button(export_label, type="primary", use_container_width=True, key="compact_export_video"):
            ...
            retry_signature = export_ai_cache_signature(
                ...
                st.session_state.get("voice_model", "Aoede"), st.session_state.get("voice_style", "cinematic narrator"),
            )
