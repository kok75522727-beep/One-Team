...
def render_voice_only_workspace(member: dict) -> None:
    ...
    if final_audio:
        ...
        with download_a:
            st.download_button("WAV Download", pcm_to_wav(final_audio), file_name="one-team-voice.wav", mime="audio/wav", use_container_width=True, key="voice_only_wav_download")
        with download_b:
            try:
                st.download_button("MP3 Download", pcm_to_mp3(final_audio), file_name="one-team-voice.mp3", mime="audio/mpeg", use_container_width=True, key="voice_only_mp3_download")
            except Exception:
                st.caption("MP3 မပြင်နိုင်သေးပါ။ WAV ကိုယူနိုင်ပါတယ်။")
        with download_c:
            if final_srt:
                st.download_button("SRT Download", final_srt.encode("utf-8"), file_name="one-team-voice.srt", mime="text/plain", use_container_width=True, key="voice_only_srt_download")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        if video_path_text:
            ...
            if final_video_for_preview:
                ...
                with final_download_left:
                    st.download_button("⬇ MP4", final_video_for_preview, file_name="one-team-movie-recap.mp4", mime="video/mp4", use_container_width=True, key="compact_final_mp4_download")
                with final_download_right:
                    compact_final_srt = normalize_srt_text(st.session_state.get("generated_srt", ""))
                    if compact_final_srt:
                        st.download_button("⬇ SRT", compact_final_srt.encode("utf-8"), file_name="one-team-burmese.srt", mime="text/plain", use_container_width=True, key="compact_final_srt_download")
                    else:
