...
def load_submitted_payment_requests() -> list[dict]:
    ...
    if client is None:
        return []
    try:
        rows = client.table("payment_requests").select("*").eq("status", "submitted").eq("plan", "pro").order("submitted_at", desc=False).execute().data or []
...
def render_pro_payment_form(member: dict) -> None:
    st.caption("Pro VIP က Owner API ဖြင့် ၄ မိနစ်အောက် Video တစ်နေ့ ၁ ပုဒ် ထုတ်လို့ရပါတယ်။")
    st.caption(f"{PRO_VIP_PRICE_MMK} · ငွေလွှဲပြီးမှ Admin စစ်ဆေးဖွင့်ပေးပါမယ်။")
...
@st.dialog("Pro VIP · 25,000 MMK")
def render_pro_upgrade_dialog(member: dict) -> None:
    render_pro_payment_form(member)
...
def render_member_admin() -> None:
    ...
    with st.expander("Admin Members"):
        ...
        if payment_requests:
            ...
            payment_plan = str(selected_payment.get("plan", "simple")).title()
            payment_days = st.number_input(f"{payment_plan} VIP Days", min_value=1, max_value=366, value=30, key="vip_payment_days")
            payment_note = st.text_input("Payment Admin Note", key="simple_payment_note")
            review_left, review_right = st.columns(2)
            with review_left:
                approve_payment = st.button(f"စစ်ပြီး {payment_plan} VIP ဖွင့်မယ်", type="primary", use_container_width=True, key="vip_payment_approve")
            with review_right:
                reject_payment = st.button("အတု/မှား · ဖျက်မယ်", use_container_width=True, key="vip_payment_reject")
            if approve_payment:
                if approve_vip_payment(int(selected_payment["id"]), selected_payment, int(payment_days), str(member.get("email", "")), payment_note):
                    st.success(f"{payment_plan} VIP ဖွင့်ပြီးပါပြီ။")
                    st.rerun()
...
def render_account_gate() -> dict:
    ...
    if member is None:
        ...
        if member:
            st.success("Simple Free ဖွင့်ပြီးပါပြီ။ Menu ထဲမှာ ကိုယ်ပိုင် Gemini Free API Key ထည့်ပါ။")
            st.rerun()
...
def main() -> None:
    ...
    plan_label = "Simple Free" if member.get("effective_plan") == "simple" else "Pro VIP"
    ...
    with simple_vip_col:
        if st.button("Simple Free", key="simple_vip", use_container_width=True):
            st.toast("Simple Free: Menu ထဲမှာ ကိုယ်ပိုင် Gemini Free API Key ထည့်ပြီးသုံးပါ။")
    with pro_vip_col:
        if st.button("Pro VIP · 25K", key="pro_vip", use_container_width=True, disabled=member.get("effective_plan") == "pro"):
            render_pro_upgrade_dialog(member)
