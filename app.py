...
def get_client(timeout_millis: int = 60000) -> genai.Client:
    ...
    return genai.Client(
        api_key=api_key,
        http_options=genai_types.HttpOptions(timeout=max(30000, int(timeout_millis))),
    )
...
FULL_VIDEO_SCRIPT_MODELS = ("gemini-2.5-flash", "gemini-3.7-flash")


def generate_full_video_script_request(uploaded, media_mime: str, prompt: str):
    """Use a full video file with the stable Flash model first; never replace it with partial frames."""
    configured_model = str(st.session_state.get("gemini_text_model", "")).strip()
    candidates = tuple(dict.fromkeys(("gemini-2.5-flash", configured_model, *FULL_VIDEO_SCRIPT_MODELS)))
    ...
    def request(client):
        ...
        for model_name in candidates:
            ...
            try:
                ...
                last_error = exc
                if "model" not in str(exc).lower() and "not found" not in str(exc).lower():
                    raise
        raise last_error or RuntimeError("Gemini Script model မရသေးပါ။")

    return call_gemini(request, attempts=2, timeout_millis=120000)
...
def generate_recap_script(video_path: Path, language: str, duration_seconds: int, tone: str, mode: str, progress_callback=None) -> str:
    ...
    uploaded = wait_for_uploaded_gemini_file(uploaded, progress_callback=progress_callback, maximum_wait_seconds=180)
