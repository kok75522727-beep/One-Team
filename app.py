...
def render_paused_frame_editor(video_path: Path, duration_seconds: float) -> None:
    """Render draggable overlays directly over a short moving clip from the uploaded video."""
    ...
    editor_scene_key = f"{video_path.stat().st_mtime_ns}:{round(frame_time, 1)}:{int(blur_enabled)}:{int(subtitle_enabled)}:{subtitle_style_revision}:{st.session_state.get('subtitle_x', 50)}:{st.session_state.get('subtitle_y', 78)}"
    ...
    editor_source_label = "မူရင်း Video" if uses_full_original else "မူရင်း Video (Editor quality)"
    st.markdown(f'<div class="mk-editor-note"><b>{editor_source_label}</b> ကို Play / Pause / Seek လုပ်လို့ရပါတယ်။ <b>Blur Box နဲ့ Subtitle ကို လက်နဲ့ရွှေ့</b>ပြီး Box အစွန်းကိုဆွဲကာအရွယ်ပြောင်းပါ။ <b>Export Video</b> နှိပ်တာနဲ့ လက်ရှိရွေးထားတာအကုန်တန်းယူပါတယ်။</div>', unsafe_allow_html=True)
...
def main() -> None:
    ...
    with editor_left:
        ...
        if video_path_text:
            ...
            with preview_placeholder.container():
                ...
                elif not overlay_editor_active:
                    render_paused_frame_editor(video_path, duration)
                else:
                    st.markdown('<div class="mk-status">Blur / Subtitle Editor ကို အောက်မှာအပြည့်ကြည့်နိုင်ပါတယ်။</div>', unsafe_allow_html=True)
    ...
    # On a phone, the editor must have the same practical width as the source
    # player; placing it below the compact controls avoids a tiny half-column.
    if video_path_text and not final_video_for_preview and overlay_editor_active:
        st.markdown('<div class="mk-panel"><div class="mk-title">Video ပေါ်မှာ Blur / Subtitle ချိန်မယ်</div></div>', unsafe_allow_html=True)
        render_paused_frame_editor(video_path, duration)
