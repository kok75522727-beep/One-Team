...
SIMPLE_TEXT_MODELS = ("gemini-3.7-flash", "gemini-2.5-flash", "gemini-2.5-flash-lite")
...
def validate_simple_free_api_key(api_key: str) -> tuple[bool, str]:
    ...
    try:
        client = genai.Client(api_key=cleaned, http_options=genai_types.HttpOptions(timeout=8000))
        result = create_simple_text_probe(client)
        del result
        return True, "Key OK · Export လုပ်လို့ရပြီ။"
    except Exception as exc:
        if is_transient_gemini_error(exc):
            return False, "Key စမ်းတာ 8 စက္ကန့်အတွင်း API မပြန်ပါ။ Internet/AI Studio Quota ကိုစစ်ပြီး ပြန်စမ်းပါ။"
...
def api_error_message(error: Exception) -> str:
    ...
    return "AI request မအောင်မြင်သေးပါ။ Key စစ်မယ်ကိုနှိပ်ပြီး မရလျှင် Google AI Studio ရဲ့ API Key/Quota ကိုစစ်ပါ။"
...
def generate_full_video_script_request(uploaded, media_mime: str, prompt: str):
    ...
    candidates = ("gemini-2.5-flash-lite",) if str(member.get("effective_plan", "")) == "simple" else FULL_VIDEO_SCRIPT_MODELS
...
def render_simple_free_key_dialog() -> None:
    ...
    if get_api_key():
        st.info("Key သိမ်းထားပြီးပါပြီ · အောက်က Key စစ်မယ် ကိုနှိပ်ပြီး API တကယ်အလုပ်လုပ်တာစစ်ပါ။")
    test_col, clear_col = st.columns(2)
    with test_col:
