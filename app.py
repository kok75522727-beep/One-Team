...
def main() -> None:
    ...
    st.session_state.setdefault("one_team_workspace", "recap")
    st.markdown('<div class="mk-workspace-strip">', unsafe_allow_html=True)
    workspace_left, workspace_right = st.columns(2, gap="small")
    with workspace_left:
        if st.button("🎬 Auto Recap", key="workspace_auto_recap", type="primary" if st.session_state.one_team_workspace == "recap" else "secondary", use_container_width=True):
            st.session_state.one_team_workspace = "recap"
            st.rerun()
    with workspace_right:
        if st.button("🎙️ Voice အသံထုတ်ရန်", key="workspace_voice_only", type="primary" if st.session_state.one_team_workspace == "voice" else "secondary", use_container_width=True):
            st.session_state.one_team_workspace = "voice"
            st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
