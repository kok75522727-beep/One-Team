...
st.set_page_config(page_title="Mg Khant · Movie Recap", page_icon="🎬", layout="wide", initial_sidebar_state="collapsed")

TURNSTILE_COMPONENT = components.declare_component(
    "mgkhant_turnstile",
...
def validate_turnstile_token(token: str) -> bool:
    """Validate one browser-issued Turnstile token on the server before creating a member record."""
    secret = membership_secret("turnstile_secret_key")
    ...
    try:
        ...
        return bool(response.ok and payload.get("success"))
...
def is_designated_admin(member: dict | None) -> bool:
    ...
    return bool(configured_email and isinstance(member, dict) and str(member.get("email", "")).lower() == configured_email)
...
def render_account_gate() -> dict:
    """Stop the editor until a Google user has passed bot verification and received admin approval."""
    ...
    if identity is None:
        st.markdown("## Mg Khant Movie Recap")
    ...
    if member is None:
        site_key = membership_secret("turnstile_site_key")
        st.markdown("### လူဖြစ်ကြောင်း အတည်ပြုပါ")
        st.caption("Google Account ဖွင့်မတိုင်ခင် Bot မဟုတ်ကြောင်း စစ်ဆေးပါ။")
        if not site_key:
            st.error("Bot စစ်ဆေးမှုကို Admin က ပြင်ဆင်နေပါတယ်။")
            st.button("Log out", on_click=st.logout)
            st.stop()
        token = TURNSTILE_COMPONENT(sitekey=site_key, key="account_turnstile")
        if token and validate_turnstile_token(str(token)):
...
def main() -> None:
    ...
    with menu_col:
        render_compact_menu()
    with brand_col:
        st.markdown(f'<div class="mk-brand"><div class="mk-brand-mark">▶</div><div><h1>Mg Khant</h1><p>{html_lib.escape(plan_label)} · Movie Recap Studio</p></div></div>', unsafe_allow_html=True)
    with simple_vip_col:
    ...
    if final_video:
        ...
        st.download_button("Final Video Download", final_video, file_name="mg-khant-movie-recap.mp4", mime="video/mp4", use_container_width=True)
        download_a, download_b = st.columns(2)
        with download_a:
            if st.session_state.get("audio"):
                st.download_button("Voice WAV Download", pcm_to_wav(st.session_state.audio), file_name="mg-khant-voiceover.wav", mime="audio/wav", use_container_width=True)
        with download_b:
            final_srt = normalize_srt_text(st.session_state.get("generated_srt", ""))
            if final_srt:
                st.download_button("SRT Download", final_srt.encode("utf-8"), file_name="mg-khant-burmese.srt", mime="text/plain", use_container_width=True)
        with st.expander("Thumbnail Prompt"):
            ...
            if st.session_state.get("thumbnail_prompt"):
                ...
                st.download_button("Prompt Download", st.session_state.thumbnail_prompt.encode("utf-8"), file_name="mg-khant-thumbnail-prompt.txt", mime="text/plain", use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
