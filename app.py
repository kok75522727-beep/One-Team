...
AZURE_PREMIUM_VOICE_CARDS = [
    ...
    ("👩‍🎤", "အေဗာ", "en-US-AvaMultilingualNeural"),
    ("👩‍🏫", "အမ်မာ", "en-US-EmmaMultilingualNeural"),
    ("👩‍🚀", "ဆယ်ရာဖီနာ", "de-DE-SeraphinaMultilingualNeural"),
    ("👩‍🎨", "ဗီဗီယန်", "fr-FR-VivienneMultilingualNeural"),
    ("👩‍💻", "ရှောင်ရှောင်", "zh-CN-XiaoxiaoMultilingualNeural"),
    ("🧑‍💼", "သီဟ", "my-MM-ThihaNeural"),
    ("🧑‍🏫", "အန်ဒရူး", "en-US-AndrewMultilingualNeural"),
    ("🧑‍🎤", "ဘရိုင်ယန်", "en-US-BrianMultilingualNeural"),
    ("🧑‍🚀", "ဖလိုရီယန်", "de-DE-FlorianMultilingualNeural"),
    ("🧑‍🎨", "ရေးမီ", "fr-FR-RemyMultilingualNeural"),
    ("🧑‍💻", "ဂျူဆက်ပီ", "it-IT-GiuseppeMultilingualNeural"),
]
...
def azure_speech_settings() -> tuple[str, str]:
    """Read Azure credentials only from secure Streamlit Secrets or deployment environment."""
    return secret_value("azure_speech", "key"), secret_value("azure_speech", "region") or "southeastasia"
...
def validate_simple_free_api_key(api_key: str) -> tuple[bool, str]:
    ...
    try:
        client = genai.Client(api_key=cleaned, http_options=genai_types.HttpOptions(timeout=30000))
        result = client.interactions.create(model="gemini-3.7-flash", input="Reply only with READY")
        del result
...
def get_client() -> genai.Client:
    ...
    # Bound an upstream wait so Cloud can show a recoverable error instead of
    # holding the export UI at the Script stage for several minutes per retry.
    return genai.Client(
        api_key=api_key,
...
def call_gemini_once(operation):
    """Run a latency-sensitive request once so a practical fallback can start promptly."""
    return operation(get_client())
...
def generate_audio_recap_script_fallback(video_path: Path, target_language: str, target_seconds: int, tone: str, progress_callback=None) -> str:
    """Use the compact dialogue track when the visual Script request times out; never affect final MP4 input."""
    audio_path = None
    try:
        if progress_callback:
            progress_callback(24, "Video စစ်တာကြာလို့ Audio နဲ့ Script ပြန်ရေးနေသည်")
        audio_path = extract_translation_audio(video_path)
        uploaded = call_gemini_once(lambda client: client.files.upload(file=str(audio_path)))
        prompt = f"""
Write a complete original movie recap narration in {target_language} from this audio track.
Target runtime: {target_seconds} seconds. Tone: {tone}.
Use only events, dialogue context, emotion, and reactions that are audible or clearly inferable from the audio.
Do not invent unseen scenes or quote dialogue word-for-word. Return only the narration with no headings, timestamps, or markdown.
"""
        interaction = call_gemini_once(lambda client: client.interactions.create(
            model=st.session_state.get("gemini_text_model", "gemini-3.7-flash"),
            input=[
                {"type": "audio", "uri": uploaded.uri, "mime_type": getattr(uploaded, "mime_type", "audio/mpeg") or "audio/mpeg"},
                {"type": "text", "text": prompt},
            ],
            generation_config={"temperature": 0.6, "thinking_level": "low"},
        ))
        text = str(getattr(interaction, "output_text", "") or "").strip()
        if not text:
            raise RuntimeError("Gemini က Audio Script မပြန်ပေးပါ။")
        return text
    finally:
        if audio_path:
            audio_path.unlink(missing_ok=True)
...
def generate_recap_script(video_path: Path, language: str, duration_seconds: int, tone: str, mode: str, progress_callback=None) -> str:
    ...
    media_type = "video"
    media_mime = mimetypes.guess_type(str(video_path))[0] or "video/mp4"
    ...
    if mode == "Faithful full translation":
        try:
            temporary_media = extract_translation_audio(video_path)
            media_path = temporary_media
            media_type = "audio"
        ...
        source_duration = get_video_duration(video_path) or 0
        if 0 < source_duration <= 180:
            try:
                if progress_callback:
                    progress_callback(10, "Script မြန်အောင် Video ကိုသေးအောင်ပြင်နေသည်")
                temporary_media = create_compact_script_analysis_video(video_path)
                media_path = temporary_media
                media_mime = "video/mp4"
            except Exception:
                temporary_media = None

    visual_setup_error = None
    try:
        if progress_callback:
            progress_callback(14, "Script အတွက် AI ဆီပို့နေသည်")
        uploaded = call_gemini(lambda client: client.files.upload(file=str(media_path)))
        if media_type == "video":
            if progress_callback:
                progress_callback(18, "Video ကို AI စစ်ဆေးနေသည်")
            uploaded = wait_for_uploaded_gemini_file(uploaded, progress_callback=progress_callback)
        elif progress_callback:
            progress_callback(22, "Script ရေးနေသည်")
    except Exception as exc:
        visual_setup_error = exc
    finally:
        if temporary_media:
            temporary_media.unlink(missing_ok=True)

    if visual_setup_error is not None:
        if mode == "Faithful full translation" or not is_transient_gemini_error(visual_setup_error):
            raise visual_setup_error
        try:
            fallback_script = generate_audio_recap_script_fallback(video_path, target_language, duration_seconds, tone, progress_callback)
            return complete_short_recap_script(fallback_script, target_language, duration_seconds, tone)
        except Exception:
            raise visual_setup_error
...
"""
    try:
        ...
        if mode == "Faithful full translation" or not is_transient_gemini_error(visual_error):
            raise
        try:
            text = generate_audio_recap_script_fallback(video_path, target_language, duration_seconds, tone, progress_callback)
        except Exception:
            raise visual_error
    if not text:
...
def generate_segmented_voiceover(script: str, voice: str, style: str, progress_callback=None, provider: str = "gemini") -> bytes:
    ...
    for index, chunk in enumerate(chunks, start=1):
        ...
        audio_segments.append(generate_azure_voiceover(chunk, voice) if provider == "azure" else generate_voiceover(chunk, voice, style))
...
def render_simple_free_key_dialog() -> None:
    ...
    test_col, clear_col = st.columns(2)
    with test_col:
        if st.button("Key စစ်မယ်", use_container_width=True, key="simple_free_direct_test_key"):
            ok, message = validate_simple_free_api_key(st.session_state.get("google_ai_key", ""))
            if ok:
                st.success(message)
            else:
                st.error(message)
...
def render_paused_frame_editor(video_path: Path, duration_seconds: float) -> None:
    ...
    editor_scene_key = f"{video_path.stat().st_mtime_ns}:{round(frame_time, 1)}:{int(blur_enabled)}:{int(subtitle_enabled)}:{subtitle_style_revision}"
    ...
    st.markdown('<div class="mk-editor-note">မူရင်း Video Frame ပေါ်မှာပါ။ <b>Blur Box နဲ့ Subtitle ကို လက်နဲ့ရွှေ့</b>ပြီး Box အစွန်းကိုဆွဲကာအရွယ်ပြောင်းပါ။ နေရာတည်ငြိမ်စေရန် ပြီးမှ <b>သိမ်းမယ်</b> ကိုနှိပ်ပါ။</div>', unsafe_allow_html=True)
...
def render_voice_only_workspace(member: dict) -> None:
    ...
    voice_text = st.text_area(
        ...
        key="voice_only_text", label_visibility="collapsed",
    )
...
def main() -> None:
    ...
    with editor_left:
        ...
        if upload:
            ...
            if st.session_state.get("last_upload_token") != upload_token or not current_source.is_file():
                activate_video_source(save_upload(upload), upload.name, upload_token)
                st.session_state.last_upload_token = upload_token
        ...
        if video_path_text:
            ...
            if final_video_for_preview:
                ...
                final_download_left, final_download_right = st.columns(2, gap="small")
                with final_download_left:
                    st.download_button("⬇ MP4", final_video_for_preview, file_name=ONE_TEAM_VIDEO_FILENAME, mime="video/mp4", use_container_width=True, key="compact_final_mp4_download")
                with final_download_right:
                    compact_final_srt = normalize_srt_text(st.session_state.get("generated_srt", ""))
                    if compact_final_srt:
                        st.download_button("⬇ SRT", compact_final_srt.encode("utf-8"), file_name=ONE_TEAM_SRT_FILENAME, mime="text/plain", use_container_width=True, key="compact_final_srt_download")
                    else:
                        st.caption("SRT မရှိသေးပါ")
