...
VOICE_CARDS = [
    ("🧑‍🎤", "ကိုခန့်", "Aoede"), ("👩‍🎤", "မသီရိ", "Kore"),
    ("🧑‍🚀", "ကိုမင်း", "Charon"), ("👩‍🚀", "မဝင်း", "Puck"),
    ("🧑‍🏫", "ကိုဇင်", "Fenrir"), ("👩‍🏫", "မသွယ်", "Enceladus"),
    ("🧑‍💼", "ကိုလင်း", "Orus"), ("👩‍💼", "မေသူ", "Schedar"),
    ("🧑‍🎨", "ကိုထက်", "Aoede"), ("👩‍🎨", "မနန်း", "Kore"),
]
VOICE_STYLE_OPTIONS = {
...
def apply_compact_ui_theme() -> None:
    ...
    st.markdown(
        ...
        div[class*="st-key-voice_only_card_"] button { min-height:4.7rem; white-space:pre-line; line-height:1.35; border-radius:13px; border-color:#314f7b; background:#101c31; font-size:.78rem; }
        ...
        @media (max-width:720px) {
          ...
          div[class*="st-key-voice_only_card_"] button { min-height:4.25rem; padding:.12rem; font-size:.63rem; }
        }
        </style>
        """,
        unsafe_allow_html=True,
...
def render_voice_only_workspace(member: dict) -> None:
    ...
    for start in range(0, len(VOICE_CARDS), 5):
        ...
        for column, (avatar, display_name, model_name) in zip(columns, VOICE_CARDS[start:start + 5]):
            with column:
                selected_mark = " ✓" if st.session_state.get("voice_only_name") == display_name else ""
                if st.button(f"{avatar}\n{display_name}{selected_mark}", key=f"voice_only_card_{display_name}", use_container_width=True):
    ...
    setting_a, setting_b, setting_c = st.columns(3, gap="small")
    with setting_a:
        speed = st.slider("အသံအမြန်", 0.75, 1.50, float(st.session_state.get("voice_only_speed", 1.0)), 0.05, key="voice_only_speed")
    with setting_b:
        pitch_label = st.radio("Pitch", list(VOICE_PITCH_OPTIONS), horizontal=True, key="voice_only_pitch")
    with setting_c:
        style_label = st.radio("Voice Style", list(VOICE_STYLE_OPTIONS), horizontal=False, key="voice_only_style")
    st.markdown('</div>', unsafe_allow_html=True)
...
def main() -> None:
    ...
    workspace = st.radio(
        "Workspace", ["🎬 Auto Recap", "🎙️ Voice အသံထုတ်ရန်"], horizontal=True,
        key="one_team_workspace", label_visibility="collapsed",
    )
    if workspace == "🎙️ Voice အသံထုတ်ရန်":
